...
impl ModuleLoader for HttpModuleLoader {
    fn load_imported_module(
        ...
        specifier: JsString,
        finish_load: Box<dyn FnOnce(boa_engine::JsResult<Module>, &mut Context<'_>)>,
...
fn main() -> JsResult<()> {
    ...
    let loader: &dyn ModuleLoader = &HttpModuleLoader;
    let queue: Rc<dyn JobQueue> = Rc::new(Queue::new(executor));
